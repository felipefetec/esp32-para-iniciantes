 ...
 * Livro: ESP32 para Iniciantes — Capítulo 12 (Desafio 1)
...
void atualizarSensor() {
  ...
  if (!isnan(t) && !isnan(u)) {
    temperatura = t;
    umidade     = u;
    // Desloca o histórico e insere a nova leitura no final
...
void handleRaiz() {
  ...
  pagina += "<meta http-equiv='refresh' content='10'><title>Estação Climática</title>";
  ...
  pagina += ".link{display:inline-block;margin-top:15px;padding:10px 20px;background:#1565c0;color:white;text-decoration:none;border-radius:6px;}";
...
void handleHistorico() {
  ...
  pagina += "<style>body{font-family:sans-serif;text-align:center;margin:0;background:#f5f5f5;}";
  pagina += "h1{background:#1565c0;color:white;padding:20px;margin:0;}";
  pagina += "table{margin:30px auto;background:white;border-radius:10px;box-shadow:0 2px 8px rgba(0,0,0,0.15);border-collapse:collapse;min-width:300px;}";
  pagina += "th{background:#1565c0;color:white;padding:12px 20px;}td{padding:12px 20px;border-bottom:1px solid #eee;}";
  pagina += ".link{display:inline-block;margin-top:15px;padding:10px 20px;background:#1565c0;color:white;text-decoration:none;border-radius:6px;}";
  pagina += "</style></head><body><h1>Histórico de Leituras</h1>";
  if (totalLeituras == 0) {
  ...
  pagina += "<br><a class='link' href='/'>Voltar</a></body></html>";
  server.send(200, "text/html", pagina);
}
